...
#import "LCPushNotification.h"
#import "LCPushCustomMessage.h"
...
/**
 消息对象
 */
@interface LCPushMessage : NSObject
 ...
 */
@property (nonatomic, strong) LCPushNotification *notification;

/**
当 OPushMessageType为OPushMessageTypeCustom时 这个字段才会有数据。
 */
@property (nonatomic, strong) LCPushCustomMessage *customMessage;

/**
 当 OPushMessageType为OPushMessageTypeAPNs时，返回apns消息数据以及场景还原数据。
 当 OPushMessageType为OPushMessageTypeLocal时，返回场景还原数据。
 "lcpush_link_k" :点击Apns消息场景还原的控制器路径。
 "lcpush_link_v" :点击Apns消息场景还原的参数。
 */
