...
- (void)testAction:(UIButton *)sender{
    ...
    if (self.tag == 1) {
        //应用内消息
        [OdinPush sendMessageWithMessageType:self.messageType
        ...
        //本地通知
        if(self.tag == 4){
            OPushMessage *message = [[OPushMessage alloc] init];
            message.messageType = OPushMessageTypeLocal;
            OPushNotification *noti = [[OPushNotification alloc] init];
...
//            noti.sound = @"unbelievable.caf";
            ...
            [OdinPush addLocalNotification:message];
            [MBProgressHUD hideHUDForView:self.view animated:YES];
